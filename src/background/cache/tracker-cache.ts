...
export class TrackerCache {
  ...
  private persistDebounceTimers = new Map<
    number,
    ReturnType<typeof setTimeout>
  >()
  private uiUpdateDebounceTimers = new Map<
    number,
    ReturnType<typeof setTimeout>
  >()
  ...
  recalculateOverallRiskScore(tabId: number): number {
    const trackerScore = calculateTrackerRiskPageScore(
      this.getTrackerDetails(tabId),
    )
  ...
  async setConsentTimingInteracted(
    tabId: number,
    onInteracted: () => Promise<void>,
  ): Promise<void> {
    const existing = this.consentTiming.get(tabId)
  ...
  async restoreFromStorage(tabId: number): Promise<void> {
    ...
    if (
      dsgvoResult != null &&
      typeof dsgvoResult === "object" &&
      !Array.isArray(dsgvoResult)
    ) {
    ...
    if (
      this.trackerDetails.has(tabId) ||
      this.cookieDetails.has(tabId) ||
      this.dsgvoResults.has(tabId)
    ) {
      this.recalculateOverallRiskScore(tabId)
    }
  }
