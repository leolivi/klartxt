...
import {
  calculateTrackerRiskPageScore,
  calculateTrackerRiskScore,
} from "./network-risk-score"

function makeTracker(
  riskScore: number,
  overrides: Partial<TrackerInfo> = {},
): TrackerInfo {
  return {
...
describe("calculateTrackerRiskScore", () => {
  ...
  it("uses the highest score when multiple categories are given", () => {
    ...
    expect(
      calculateTrackerRiskScore(
        [TrackerCategory.MALWARE, TrackerCategory.CDN],
        TrackerConfidence.CONFIRMED,
      ),
    ).toBe(100)
  })
  ...
  it("uses SUSPICIOUS multiplier with multiple categories", () => {
    ...
    expect(
      calculateTrackerRiskScore(
        [TrackerCategory.AD, TrackerCategory.ANALYTICS],
        TrackerConfidence.SUSPICIOUS,
      ),
    ).toBe(35)
  })
...
describe("calculateTrackerRiskPageScore", () => {
  ...
  it("calculates score for two trackers", () => {
    ...
    expect(
      calculateTrackerRiskPageScore([makeTracker(100), makeTracker(50)]),
    ).toBe(87)
  })
