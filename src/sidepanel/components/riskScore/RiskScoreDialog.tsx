      ...
      {LEVELS.map((level, i) => (
        <div key={level}>
          <div className="flex gap-3 items-center py-3">
            <div
              className={`rounded-full size-8 shrink-0 flex items-center justify-center font-semibold ${LEVEL_COLORS[level]}`}
            >
              {level}
            </div>
            <div>
...
export function RiskScoreDialog() {
  ...
  return (
    <Dialog>
      <DialogTrigger aria-label={t("riskScoreDialogOpenLabel")}>
        <Info
          aria-hidden="true"
          className="absolute top-[0.1rem] right-[-0.4rem] stroke-ink-strong bg-surface-primary hover:bg-surface-tertiary rounded-full"
          size={25}
        />
      </DialogTrigger>
      <DialogContent>
