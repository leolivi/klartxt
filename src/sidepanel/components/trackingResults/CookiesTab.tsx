...
export function CookiesTab() {
  ...
  const byCategory = cookiesList.reduce<Record<string, ClassifiedCookie[]>>(
    (acc, cookie) => {
  ...
  const categories = Object.keys(byCategory).sort(
    (a, b) =>
      (CATEGORY_ORDER[a as CookieCategoryForUser] ?? 99) -
      (CATEGORY_ORDER[b as CookieCategoryForUser] ?? 99),
  )
  ...
  return (
    <Accordion type="multiple">
      {categories.map((cat, i) => {
        ...
        const byDomain = cookies.reduce<Record<string, ClassifiedCookie[]>>(
          (acc, cookie) => {
          ...
          <div key={cat}>
            <AccordionItem value={cat} className="border-b-0">
              <AccordionTrigger>
                ...
                <span className="ml-auto mr-2 text-small text-ink-strong">
                  {cookies.length}
                </span>
              </AccordionTrigger>
              <AccordionContent>
                  ...
                  {Object.entries(byDomain).map(([domain, domainCookies]) => (
                    <div key={domain}>
                      <div className="flex items-center gap-1">
                        ...
                        <p className="text-small text-ink-strong pb-1">
                          {domain}
                        </p>
                      </div>
