...
const FINGERPRINT_DOMAIN_SET = new Set<string>(
  Object.keys(fingerprintData.domains),
)
...
function hasFingerprintScript(): boolean {
  return Array.from(document.querySelectorAll("script[src]")).some((s) => {
    try {
      ...
      return (
        FINGERPRINT_DOMAIN_SET.has(hostname) ||
        FINGERPRINT_DOMAIN_SET.has(registrable)
      )
    } catch {
      return false
    }
