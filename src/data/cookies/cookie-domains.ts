...
export function mapToUserCategory(
...
export function mapNameToCategory(name: string): CookieCategory {
  ...
  if (NECESSARY_MIN3.some((p) => lower.includes(p)))
    return CookieCategory.NECESSARY
  if (SESSION_MIN3.some((p) => lower.includes(p))) return CookieCategory.SESSION
  if (ANALYTICS_MIN3.some((p) => lower.includes(p)))
    return CookieCategory.ANALYTICS
  if (ADVERTISING_MIN3.some((p) => lower.includes(p)))
...
function mapTrackerCategoryToCookieCategory(
...
export function classifyCookieCategory(
  ...
  if (trackerInfo) {
    return mapTrackerCategoryToCookieCategory(
      trackerInfo.detailedCategories[0] ?? "unknown",
    )
  }
